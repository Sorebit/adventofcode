...
from aoc import ints_g, lmap, read_lines, vpprint, vprint
...
def part_one(all_lines: list[str]):
    result_1 = 0
    gen_nums = [ints_g(line) for line in all_lines[:-1]]
...
def part_two(all_lines: list[str]):
    result_2 = 0
    print("p2")
    gen_nums = [iter(line) for line in all_lines[:-1]]
    gen_symbols = find_all_symbols(all_lines[-1])
...
def solve(in_file: Path):
    ...
    all_lines = list(read_lines(in_file, strip=False))
